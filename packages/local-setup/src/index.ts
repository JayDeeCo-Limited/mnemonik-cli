...
import { isCanonicalUuid, resolveProjectIdentity } from '@mnemonik/shared';
...
export interface SetupRecord {
  ...
  ignored?: { identityHash: string | null; responseHash: string };
  evidence?: Evidence;
  remote?: { projectId: string; displayName?: string };
  before: { base64: string | null; hash: string | null };
  staged?: { content: string; hash: string };
  steps: {
    remote: Step & { outcome?: 'created' | 'linked' | 'restored' };
    identity: Step;
    rollback: Step;
  };
}
...
const validDisplayName = (value: unknown): value is string =>
  typeof value === 'string' && value.length <= 200 && !/[\p{C}\p{Zl}\p{Zp}@]/u.test(value);
const digest = (bytes: Buffer | null): string | null => (bytes === null ? null : hash(bytes));
const accessResponseHash = (response: {
  status: string;
  state: string;
...
const nonGitSelectionRequired = (): SetupResult => ({
...
const recordedCandidate = (
  remote: NonNullable<SetupRecord['remote']>,
  root: string,
  state: 'confirmation_required' | 'fingerprint_mismatch'
): SetupResult => ({
...
export function createProjectSetupExecutor(deps: ExecutorDependencies) {
  async function execute(
    options: EnsureOptions,
    mode: 'ensure' | 'stage' | 'apply' | 'rollback'
  ): Promise<SetupResult> {
    ...
    if (resolution.repository.kind === 'plain' && !options.nonGitSelected) {
      ...
      if (!prior) return nonGitSelectionRequired();
      try {
        if ((JSON.parse(prior.toString()) as SetupRecord).nonGitSelected !== true)
    ...
    return withLock(path, deps.waitMs ?? 60_000, async (assertOwned) => {
      ...
      if (fresh.kind !== 'ok' && fresh.kind !== 'absent') return actionRequired(fresh.kind);
      if ((await realpath(fresh.root)) !== root) return actionRequired('identity_changed');
      resolution = fresh;
      ...
      let permissionStatus: PermissionStatus = 'private';
      const save = async () => {
        permissionStatus = await atomicWrite(
          path,
          Buffer.from(JSON.stringify(record, null, 2) + '\n'),
          undefined,
          assertOwned
      ...
      const result = (status: 'done' | 'staged' | 'rolled_back' | 'ignored'): SetupResult => ({
        status,
      ...
      const handleAccessResponse = async (
        response: Extract<SetupResult, { status: 'ACTION_REQUIRED' | 'project_setup_required' }>
      ): Promise<SetupResult> => {
        ...
        if (!record.ignored) return response;
        if (
          !options.clearIgnore &&
      ...
      if (mode === 'rollback') {
        ...
        if (diskHash !== record.before.hash) {
          if (record.before.base64 === null) {
            ...
            await atomicWrite(
              file,
              Buffer.from(record.before.base64, 'base64'),
              deps.fault,
              assertOwned
            );
        }
      ...
      if (record.steps.rollback.started)
        return actionRequired(
          record.steps.rollback.complete ? 'operation_rolled_back' : 'rollback_in_progress'
        );
      ...
      const resumingWithoutIdentity =
        !!remote && diskHash === record.before.hash && diskHash !== record.staged?.hash;
      ...
      if (record.steps.identity.complete && mode === 'ensure' && record.remote) {
        ...
        if (validated.status !== 'complete') return handleAccessResponse(validated);
        await clearIgnore();
        if (validated.projectId !== record.remote.projectId)
          return actionRequired('identity_changed');
      }
      if (!record.remote) {
        if (mode === 'apply') return actionRequired('not_staged');
        ...
        if (issued.status !== 'complete') {
          ...
          if (handled.status === 'ignored' || issued.status === 'ACTION_REQUIRED') return handled;
        } else await clearIgnore();
        let remote;
        let outcome: 'created' | 'linked' | 'restored' = 'linked';
        if (issued.status === 'complete') remote = issued;
        else if (
          options.intent &&
        ...
        if (!isCanonicalUuid(remote.projectId)) return actionRequired('invalid_server_result');
        record.remote = {
          projectId: remote.projectId,
      ...
      if (mode === 'stage') return result('staged');
      if (current !== record.staged.hash) {
        if (current !== record.before.hash) return actionRequired('identity_changed');
  ...
  return {
    ensureProject: (options: EnsureOptions) => execute(options, 'ensure'),
