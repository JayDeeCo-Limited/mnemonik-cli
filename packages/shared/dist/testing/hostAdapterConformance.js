...
export function hostAdapterConformance(create, options) {
    ...
    const fixture = async () => {
        ...
        return {
            ...
            deps,
            adapter: create(deps),
            configPath: (scope) => join(scope === 'user' ? home : project, options.config),
        };
    };
    const checks = {};
    ...
    const assertForeign = async (path) => {
    ...
    check('detect reports every searched location and writes nothing with host binaries absent', async ({ root, adapter }) => {
    ...
    if (options.deferred) {
        check('defers every other operation', async ({ adapter, target }) => {
            for (const operation of [
                ...
                await assert.rejects(async () => operation(), /not_supported/);
        });
        return checks;
    }
    const versionOutput = {
    ...
    check('Windows prefers vendor installation, then filters ordered where candidates and caches the result', async ({ deps, target }) => {
        const name = options.name === 'claude-code' ? 'claude' : options.name;
        const extension = options.name === 'grok' ? '.exe' : '.cmd';
        const preferred = options.name === 'cursor'
        ...
        for (const installed of [true, false]) {
            ...
            assert.equal(calls.filter(([file]) => file.endsWith('where.exe')).length, installed ? 0 : 1);
    ...
    for (const scope of options.scopes)
        check(`${scope}: deterministic plan; install stages and does not write; uninstall preserves foreign bytes`, async ({ root, target: base, deps, configPath }) => {
            const adapter = create({
                ...deps,
            ...
            await writeChanges([
                {
                    ...
                    content: Buffer.from(`${JSON.stringify({ hooks: { [event]: [foreignEntry] } }, null, 2)}\n`),
                },
            ]);
            const before = await snapshot(root);
            ...
            if (options.name === 'codex') {
                ...
                const binding = plan.changes.find((change) => change.path === join(target.runtimeRoot, 'binding.json'));
                assert.ok(binding);
                ...
                assert.ok(plan.changes.some((change) => change.content.includes('--credential-family ' + target.credentialFamily)));
            ...
            if (options.name !== 'codex') {
                ...
                assert.ok(hooks[event]
                    ...
                    .some((entry) => entry.command?.includes('--credential-family ' + target.credentialFamily) &&
                    (entry.timeout ?? 0) >= 40));
            }
            if (options.name !== 'codex')
    ...
    for (const scope of options.scopes)
        for (const component of options.mcpConfig ? ['hooks', 'mcp'] : ['hooks'])
            check(`${scope} ${component}: uninstall removes only a config created by install`, async ({ target: base, deps, project, configPath }) => {
                const target = { ...base, component, scope };
                const path = component === 'hooks'
                    ? configPath(scope)
                    : join(scope === 'user' ? (deps.env?.HOME ?? '') : project, options.mcpConfig?.[scope] ?? '');
                ...
                staged.length = 0;
                await preExisting.uninstall({ stage: async (change) => void staged.push(change) }, preExistingTarget);
                await writeChanges(staged);
                assert.ok(!(await readFile(path, 'utf8')).includes('mnemonik'));
            });
    check('replacement rule: reports alternate-scope entries and never stages that config', async ({ adapter, target, configPath }) => {
        await writeChanges((await adapter.plan(target)).changes);
        const before = await readFile(configPath('user'));
        const projectTarget = { ...target, scope: 'project' };
        assert.deepEqual((await adapter.inspect(projectTarget)).otherScopes, [
            { scope: 'user', path: configPath('user') },
        ]);
        assert.ok(!(await adapter.plan(projectTarget)).changes.some((c) => c.path === configPath('user')));
        await writeChanges((await adapter.plan(projectTarget)).changes);
        assert.deepEqual(await readFile(configPath('user')), before);
    });
    if (options.mcpConfig)
        for (const scope of options.scopes)
            check(`${scope}: MCP declaration has the correct machine header and preserves foreign fields`, async ({ root, project, deps, target: base }) => {
                const target = { ...base, component: 'mcp', scope };
                const adapter = create({ ...deps, target });
                ...
                const withoutSessionHeader = options.name === 'grok' && config.toml
                    ? raw.replace(/,?\s*"x-mcp-session-id"\s*=\s*"\{\{session_id\}\}"/u, '')
                    : raw;
                if (options.name === 'grok')
                    assert.match(raw, sessionHeader);
                if (scope === 'user')
                    assert.match(withoutSessionHeader, /x-mnemonik-installation-id/);
                else
                    assert.doesNotMatch(withoutSessionHeader, /x-mnemonik-installation-id/);
                if (config.toml)
                    assert.ok(raw.startsWith(original));
                else {
                ...
                if (scope === 'user') {
                    ...
                    if (config.toml)
                        await writeFile(path, installed
                            ...
                            .replace(/\{\s*,/u, '{')
                            .replace(/,\s*\}/u, ' }')
                            .replace('url = "https://api.mnemonik.dev/mcp"\n', 'url = "https://api.mnemonik.dev/mcp"\ncustom = "keep exactly"\n'));
                    else {
                    ...
                    if (options.name === 'grok')
                        assert.match(repaired, /x-mcp-session-id/);
                    if (config.toml) {
                ...
                if (config.toml)
                    ...
                    assert.deepEqual(JSON.parse(removed).mcpServers.foreign, foreign);
                ...
                await assert.rejects(adapter.uninstall({ stage: async () => assert.fail('must not stage') }, target), /mcp_name_conflict/);
                assert.deepEqual(await snapshot(root), conflicted);
            });
    check('native login/logout use the named server; missing executable returns manual instruction without writes', async ({ root, deps }) => {
        const calls = [];
        const adapter = create({
            ...deps,
...
//# sourceMappingURL=hostAdapterConformance.js.map