...
export function hostAdapterConformance(create, options) {
    ...
    check('parses connected listings without inferring account and uses execFile arguments', async ({ deps, project, target }) => {
    ...
    if (options.mcpConfig)
        for (const scope of options.scopes)
            check(`${scope}: MCP URL only, foreign server preserved, staged lifecycle`, async ({ root, project, deps, target: base }) => {
                ...
                const withoutSessionHeader = options.name === 'grok' && config.toml ? raw.replace(sessionHeader, '') : raw;
                if (options.name === 'grok')
                    assert.ok(raw.includes(sessionHeader));
                assert.doesNotMatch(withoutSessionHeader, /token|secret|headers|credential|bearer|runtime|client.?id/i);
                if (config.toml)
