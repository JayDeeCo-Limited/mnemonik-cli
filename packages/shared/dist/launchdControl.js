...
export async function printLaunchdRegistration(domain, label, run) {
    try {
...
export async function processesAlive(pids, run) {
    const list = [...pids];
    if (!list.length)
        return false;
    try {
...
export async function authorizeMacService(run, env = process.env) {
    ...
    try {
        ...
        throw new Error(!interrupted &&
            /terminal is required|a password is required|no tty present|no askpass program/iu.test(error.message)
...
export async function removeMacScanner(state, home, uid, run, options = {}) {
    const exists = (path) => lstat(path).then(() => true, (error) => {
        if (error.code === 'ENOENT')
            return false;
        throw error;
    });
    const receipt = await readFile(join(state, 'scanner/status.json'), 'utf8').catch((error) => {
        if (error.code === 'ENOENT')
            return null;
        throw error;
    });
    ...
    if (system || present || launcherPresent)
        await authorizeMacService(run, options.environment);
    const privileged = (file, args) => file === 'launchctl' && args[0] === 'bootout'
    ...
    if (options.remove === false) {
        ...
        await writeFile(join(state, 'scanner/service-replacement/result.json'), JSON.stringify({ stopped: true }));
        return;
    }
    if (present || launcherPresent)
        await run('sudo', ['-n', '/bin/rm', '-f', plist, launcher]);
...
export async function stopLaunchdRegistrations(domains, label, run, options = {}) {
    const now = options.now ?? Date.now;
    const sleep = options.sleep ?? delay;
    const inspect = (domain) => printLaunchdRegistration(domain, label, run);
    const pids = new Set();
    ...
    do {
        ...
        if (!registered && !(await processesAlive(pids, run)))
            return;
        await sleep(100);
    } while (now() <= deadline);
    throw new Error('scanner_stop_failed');
}
//# sourceMappingURL=launchdControl.js.map