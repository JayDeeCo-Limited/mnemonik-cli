...
export function protectedLocalPaths(platform = process.platform, env = process.env, home = homedir()) {
    ...
    const state = env.MNEMONIK_STATE_DIR ||
        (platform === 'win32'
...
export function isProtectedLocalPath(candidate, protectedPaths = protectedLocalPaths(), platform = process.platform) {
    const path = pathsFor(platform);
...
export function protectedPathsWithinRoot(root, protectedPaths = protectedLocalPaths(), platform = process.platform) {
