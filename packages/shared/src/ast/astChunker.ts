 ...
 */
const KIND_SPECIFICITY: Readonly<Record<string, number>> = {
  constant: 1,
  variable: 1,
  field: 1,
  property: 1,
  ...
  for (const match of query.matches(root)) {
    ...
    if (existing) {
      ...
      if (named !== undefined && existing.name !== undefined && existing.name !== named) {
        ...
        if (incomingPri > existingPri) existing.kind = kind;
        if (named && !existing.name) existing.name = named;
      }
      continue;
    }
...
function buildChunks(definitions: Definition[], lines: string[]): BuiltChunks {
  ...
  const containerName = new Array<string | undefined>(definitions.length);
  const open: number[] = [];
  for (let i = 0; i < definitions.length; i++) {
    ...
    if (parentIndex !== undefined && parent) {
      if (suppressed[parentIndex] === 1 || !STRUCTURAL_KINDS.has(parent.kind)) {
        ...
        if (parent.name) containerName[i] = parent.name;
