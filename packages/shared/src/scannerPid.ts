...
export function pidIsScanner(
  pid: number,
  platform = process.platform,
  exec = execFileSync,
  identity?: { binaryPath: string; uid: number }
): boolean {
  const cmdline = processCmdline(pid, platform, exec);
  if (cmdline === null) return false;
  if (!identity) return SCANNER_CMD_PATTERN.test(cmdline);
  // Recovery may signal only the exact verified runtime, running as this owner.
  if (cmdline !== `${identity.binaryPath} start`) return false;
  try {
