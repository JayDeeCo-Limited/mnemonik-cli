...
/**
 * Mutable per-walk state threaded through the recursive traversals.
 * `complete` starts true and is latched false by any caught fs error
 * (readdir failure, per-entry lstat/realpath/stat throw) - meaning the
 * result may be missing entries that still exist on disk. Callers that
 * derive *removals* from a walk must treat an incomplete walk as
 * untrustworthy; adds/changes are unaffected.
 */
interface WalkState {
  complete: boolean;
}
 ...
 */
export const BUILT_IN_IGNORE_DIRS: readonly string[] = [
  // JS / web
  'node_modules',
  '.next',
  '.nuxt',
...
const BUILT_IN_IGNORE_DIR_SET = new Set<string>(BUILT_IN_IGNORE_DIRS);
 ...
 */
export async function isGitBoundary(dirPath: string): Promise<boolean> {
  try {
    await withTimeout(
      lstat(join(dirPath, '.git')),
      FILE_OP_TIMEOUT_MS,
      `lstat timed out: ${join(dirPath, '.git')}`
    );
    return true;
  } catch {
    return false;
...
/**
 * One directory's compiled ignore-file rules, keyed by its path relative to
 * the scan root (`''` = root). A stack of these, root->leaf, is threaded
 * through the walkers; `ignoredByStack` tests deepest-first so a nested
 * `.gitignore` overrides a shallower one (git precedence).
 */
...
export class CodeScanner {
  ...
  readonly skipReasons = { symlink_skipped: 0 };

  private skipSymlink(stats: Stats): boolean {
    if (!stats.isSymbolicLink()) return false; // Includes Windows junctions.
  ...
  private async isProtectedRead(path: string): Promise<boolean> {
    if (this.isProtected(path)) return true;
    ...
    for (const name of IGNORE_FILE_NAMES) {
      ...
      try {
        const content = await withTimeout(
          this.readConfined(filePath, rootPath),
          FILE_OP_TIMEOUT_MS,
   ...
   */
  private async buildIgnoreStackForFile(
    fileRel: string,
    rootPath: string,
    cache: Map<string, IgnoreLayer | null>,
    walk: WalkState
  ): Promise<IgnoreStack> {
    const dirs = [''];
    let acc = '';
    ...
    for (const dir of dirs) {
      ...
      if (layer === undefined) {
        layer = await this.loadIgnoreLayer(
          dir === '' ? rootPath : join(rootPath, dir),
          dir,
          walk,
          rootPath
        );
   ...
   */
  private async insideNestedGitBoundary(filePath: string, rootPath: string): Promise<boolean> {
    ...
    for (let i = 0; i < segments.length - 1; i++) {
      ...
      if (await isGitBoundary(current)) return true;
    }
    return false;
  }
  ...
  private canonicalRoots = new Map<string, string>();
  ...
  static containsCanonical(
    ...
    return (
      candidate === root || candidate.startsWith(root.endsWith(paths.sep) ? root : root + paths.sep)
    );
  }

  private async confineSymlink(fullPath: string, canonicalRoot: string): Promise<boolean> {
    ...
    if (!CodeScanner.containsCanonical(path, canonicalRoot)) return false;
    for (;;) {
      if (this.skipSymlink(await lstat(path))) throw new Error('symlink_skipped');
      if (CodeScanner.containsCanonical(canonicalRoot, path)) break;
      path = dirname(path);
    }
    return CodeScanner.containsCanonical(await realpath(fullPath), canonicalRoot);
  }

  async readConfined(fullPath: string, canonicalRoot: string, enumerated?: Stats): Promise<string> {
    const expected = enumerated ?? (await lstat(fullPath));
    if (this.skipSymlink(expected)) throw new Error('symlink_skipped');
    if (!expected.isFile()) throw new Error('scanner_not_regular');
    ...
    if (
      !CodeScanner.containsCanonical(canonical, canonicalRoot) ||
    ...
    const handle = await open(fullPath, flags).catch((error: NodeJS.ErrnoException) => {
      if (error.code === 'ELOOP') {
        this.skipReasons.symlink_skipped++;
    ...
    try {
      ...
      if (!actual.isFile()) throw new Error('scanner_not_regular');
  ...
  private async traversePaths(
    ...
    const dirRel =
      currentPath === rootPath ? '' : relative(rootPath, currentPath).split(sep).join('/');
    ...
    for (const entry of entries) {
      ...
      if (this.isProtected(fullPath)) continue;
      ...
      try {
        ...
        if (this.skipSymlink(lstats)) continue;
        const stats = lstats;
  ...
  async scanFiles(filePaths: string[], rootPath: string): Promise<CodeChunk[]> {
    ...
    for (const filePath of filePaths) {
      try {
        if (!(await this.confineSymlink(filePath, rootPath))) continue;
        if (this.isProtected(filePath) || (rootPath && this.isProtected(rootPath))) continue;
   ...
   */
  private scrubChunks(chunks: CodeChunk[]): CodeChunk[] {
  ...
  private async parseFile(
    ...
    try {
      ...
      let content = await withTimeout(
        this.readConfined(filePath, rootPath, stats),
        FILE_OP_TIMEOUT_MS,
        `readFile timed out: ${filePath}`
      );
      ...
      if (astLanguage) {
        ...
        if ('chunks' in ast) {
          ...
          if (bounded) {
            ...
            return bounded.map(
              ({ symbolName, symbolKind, signature, symbolContainer, ...chunk }) => ({
                ...chunk,
                filePath: relativePath,
                language,
  ...
  private findMatchingBrace(content: string, openIndex: number): number {
    ...
    while (i < len) {
      ...
      if (c === '/' && i > 0) {
        ...
        let j = i - 1;
        while (j >= 0 && (content.charAt(j) === ' ' || content.charAt(j) === '\t')) j--;
        ...
        if (!isRegexContext && j >= 0 && /[a-z]/.test(prev)) {
          ...
          let wordStart = j;
          while (wordStart > 0 && /[a-z]/.test(content.charAt(wordStart - 1))) wordStart--;
   ...
   */
  async collectAuthorityFilesWithStatus(
    projectRoot: string,
    onFile?: (file: { path: string; content: string; hash: string }) => Promise<void>
  ): Promise<{
    ...
    const walk = async (dir: string, depth: number, stack: IgnoreStack = []): Promise<void> => {
      if (depth >= CodeScanner.MAX_DEPTH) return;
      if (this.isProtected(dir)) return;
      let entries;
      try {
        if (!(await this.confineSymlink(dir, projectRoot))) throw new Error('scanner_containment');
      ...
      for (const ent of entries) {
        ...
        if (this.skipSymlink(stats)) continue;
        const isDir = stats.isDirectory(),
          isFile = stats.isFile();

        if (this.ignoredByStack(rel, isDir, localStack)) continue; // .gitignore/.mnemonikignore
        if (isDir) {
          if (await isGitBoundary(full)) continue; // another repo's subtree
          await walk(full, depth + 1, localStack);
        } else if (isFile && AUTHORITY_FILE_MATCHERS.some((m) => m(rel))) {
          let file: { path: string; content: string; hash: string } | undefined;
          try {
