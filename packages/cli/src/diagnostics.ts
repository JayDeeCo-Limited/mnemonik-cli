...
interface PreviewReceipt extends PreviewResult {
  schemaVersion: 1;
}
...
const validId = (id: string): boolean => /^diag-[A-Za-z0-9._-]{1,120}$/u.test(id);

async function scannerConfig(stateDir: string): Promise<{
  serverUrl: string;
  credentialFamilyId: string;
}> {
  const state = JSON.parse(await readFile(join(stateDir, 'scanner', 'state.json'), 'utf8')) as {
    config?: { serverUrl?: unknown; credentialFamilyId?: unknown };
  };
  if (
    typeof state.config?.serverUrl !== 'string' ||
...
function rotationTransport(serverUrl: string, fetcher: typeof fetch): CredentialTransport {
  const request = async (path: string, refreshToken: string) => {
  ...
  return {
    rotateFamily: (id, token) =>
      request(`/api/v1/component-credentials/${encodeURIComponent(id)}/rotate`, token),
    revokeFamily: (id, token) =>
      request(`/api/v1/component-credentials/${encodeURIComponent(id)}/revoke`, token),
  };
}
...
export async function previewDiagnostics(
  ...
  try {
    result = JSON.parse(stdout) as PreviewResult;
  } catch {
    throw new DiagnosticsError('diagnostics_preview_invalid');
  }
  if (
    !validId(result.manifest?.bundleId) ||
    !/^[a-f0-9]{64}$/u.test(result.sha256) ||
    typeof result.path !== 'string'
  )
    throw new DiagnosticsError('diagnostics_preview_invalid');
  const receipt: PreviewReceipt = { schemaVersion: 1, ...result };
...
): Promise<WorkResponse<Record<string, unknown>>> {
...
export async function sendDiagnostics(
  ...
  const receipt = JSON.parse(await readFile(receiptPath, 'utf8')) as PreviewReceipt;
  if (
  ...
  if (digest(bytes) !== receipt.sha256) throw new DiagnosticsError('bundle_hash_mismatch');
  ...
  const response = await credentials.withCredential(
    config.credentialFamilyId,
    dependencies.rotation ?? rotationTransport(config.serverUrl, fetcher),
    (token) =>
      uploadDiagnosticsBundle(config.serverUrl, token, bundleId, receipt.sha256, bytes, fetcher)
  );
  if (!('body' in response)) throw new DiagnosticsError(`credential_${response.reason}`);
