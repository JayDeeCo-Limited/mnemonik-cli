...
const shellWord = (value: string): string =>
  /^[\w@%+=:,./-]+$/u.test(value) ? value : `"${value.replace(/(["\\$`])/gu, '\\$1')}"`;

/**
 * The command that gives each offered answer, for an agent that reads JSON.
 * An action no command can give is left out.
 */
export function answerCommands(
  actions: readonly string[],
  root: string,
  details: { candidates?: Array<{ projectId: string }>; projectId?: string } = {}
): Array<{ action: string; command: string }> {
  const folder = shellWord(root);
  const linkFlags = (projectId: string, extra: string) =>
    `mnemonik project link ${projectId} ${folder}${extra} --non-interactive --apply`;
...
const refusalReasons: Record<string, string> = {
  ...
  fingerprint_mismatch:
    'Its Git remote does not match the repository this project was set up with.',
...
const refusalSteps: Record<string, string> = {
...
export const hasRefusalWords = (reason: string): boolean => reason in refusalReasons;

/** No bare line: name the folder, say why in plain words, give the one command. */
export function folderRefusalMessage(reason: string, root: string): string[] {
 ...
 */
const choiceLines = (actions: readonly string[]): string[] =>
...
function actionRequired(
  ...
  if (json) output.json(result);
    ...
    if (Array.isArray(details.allowedActions))
      for (const line of choiceLines(details.allowedActions.map(String))) output.line(line);
  }
  return 3;
}
...
function showResult(
  ...
  if (json)
    ...
    if (hasRefusalWords(state))
      for (const line of folderRefusalMessage(state, root)) output.line(line);
    else output.error(humanReason(state));
    ...
    if ('allowedActions' in result)
      for (const line of choiceLines(result.allowedActions)) output.line(line);
  }
}
...
async function cancelCommand(
  ...
  const resolution = await (
    deps.executor ??
    deps.resolver ?? { resolveProjectIdentity }
  ).resolveProjectIdentity(cwd, { allowNestedInherit: false });
  ...
  if (input.json) deps.output.json(result);
    ...
    deps.output.line(
      result.cleared ? `Setup of ${name} was cancelled.` : `Nothing was waiting for ${name}.`
    );
 ...
 */
function accountFailure(error: unknown): string | undefined {
  if (error instanceof ServerActionRequiredError) return error.result.state;
  if (error instanceof Error && error.message === 'invalid_request') return 'invalid_request';
  return undefined;
}
 ...
 * error.
 */
async function handoffRequestId(input: Readable): Promise<string | undefined> {
  if ((input as Readable & { isTTY?: boolean }).isTTY) return undefined;
  let text = '';
  ...
  let value: { requestId?: unknown } | null;
  try {
    value = JSON.parse(text) as { requestId?: unknown } | null;
  } catch {
