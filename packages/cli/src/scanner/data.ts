...
export async function deleteScannerIndex(projectId: string, bearer: string, fetcher = fetch) {
  if (!/^[0-9a-f-]{36}$/i.test(projectId)) throw new Error('invalid_project_id');
  const request = async (method: string) => {
    const response = await fetcher(
      `${apiOrigin()}/api/v1/projects/${encodeURIComponent(projectId)}/index`,
      {
        method,
        headers: { Authorization: `Bearer ${bearer}` },
      }
    );
    if (!response.ok) throw new Error(`index_${method.toLowerCase()}_failed_${response.status}`);
