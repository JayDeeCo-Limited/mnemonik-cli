...
export const DEVICE_WARNING =
  "Approve only a request you just started on a device you control. Compare this code with the one in that device's terminal. If you received this code or link from someone else, deny it.";
...
export async function runDeviceFlow(options: DeviceOptions): Promise<DeviceResult> {
  ...
  while (now() < deadline) {
    ...
    if (error === 'expired_token') throw new OAuthProtocolError(error, 'expired');
