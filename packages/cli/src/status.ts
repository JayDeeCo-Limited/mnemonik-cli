...
export function buildStatusDocument(
  input: StatusDocumentInput
): ReadinessDocument & { conditions: ReadinessCondition[] } {
 ...
 */
function messageFor(reason: string, actions: readonly string[], action?: string) {
...
export function renderStatusSummaries(
  ...
  if (project) {
    ...
    else renderAttention('This project', project.summary, output, rendered, document.conditions);
  }
}
...
export function refusalLines(
  batches: ReadonlyArray<{ project: string; files: number; issue: string }> | undefined,
  withPaths = false
): string[] {
  const projects = new Map<string, { files: number; issues: Set<string> }>();
  for (const { project, files, issue } of batches ?? []) {
    const entry = projects.get(project) ?? { files: 0, issues: new Set<string>() };
    entry.files += files;
...
export function scannerFailureCause(
  failure: NonNullable<ScannerReceipt['snapshot']['failure']>
): string {
  switch (failure.kind) {
 ...
 */
export async function renderScannerFailure(
  stateDir: string,
  output: Pick<Output, 'line'>
): Promise<void> {
...
export async function renderRefusals(
  stateDir: string,
  output: Pick<Output, 'line'>,
  withPaths = false
): Promise<void> {
...
> {
  ...
  let scannerFailing: ReadinessCondition | undefined;
  const restarted =
    !input.scannerStatus &&
  ...
  if (
    ...
    (await resumeAbandonedPause({ ...input.abandonedPause, stateDir: statusStateDir }).catch(
      () => false
    ))
  )
  ...
  const candidate =
    ...
    (resolved.identity !== 'absent' ||
      (await stat(join(resolved.resolvedRoot, '.git')).then(
        () => true,
        () => false
      )) ||
