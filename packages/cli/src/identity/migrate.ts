...
type SourceStatus = { name: string; path: string; status: 'found' | 'absent' | 'unreadable' };
...
export interface IdentityReport {
  schemaVersion: 1;
  generatedAt: string;
  host: { platform: NodeJS.Platform; home: string; stateDirectory: string };
...
export type MigrationResult =
  | { status: 'reported'; report: IdentityReport }
  | { status: 'backed_up'; runId: string; indexPath: string; count: number; report: IdentityReport }
  | {
...
const digest = (bytes: Buffer | string): `sha256:${string}` =>
...
function pathStrings(value: unknown, key = ''): string[] {
  if (typeof value === 'string')
    return /(?:cwd|path|project|workspace|root|folder)/iu.test(key) && isAbsolute(value)
      ? [value]
      : [];
  if (Array.isArray(value)) return value.flatMap((item) => pathStrings(item, key));
...
function pathsFromText(text: string): string[] {
  try {
    ...
    const assignments = [
      ...text.matchAll(
        /^\s*(?:cwd|path|project|workspace|root|folder)[\w.-]*\s*=\s*["']([^"']+)["']/gimu
      ),
...
async function addConfig(
  ...
  try {
    ...
    for (const candidate of pathsFromText(raw)) addCandidate(candidates, candidate, name);
  } catch (error) {
    sourceStatuses.push({
      name,
      path,
...
function addCandidate(map: Map<string, Set<string>>, raw: string, source: string): void {
  const path = resolve(raw.endsWith(IDENTITY) || raw.endsWith(RULE) ? dirname(raw) : raw);
  const sources = map.get(path) ?? new Set<string>();
  sources.add(source);
  map.set(path, sources);
}

async function addRecordPaths(
  ...
  for (const [name, directory, nested] of [
    ...
    for (const first of await files(directory)) {
      ...
      for (const relative of targets.filter((x) => x.endsWith('.json'))) {
        found = true;
        try {
          const record = JSON.parse(await readFile(join(directory, relative), 'utf8')) as Record<
            string,
            unknown
          >;
          const root = typeof record.root === 'string' ? record.root : record.resolvedRoot;
          if (typeof root === 'string') addCandidate(map, root, name);
        } catch {
...
async function collectCandidates(options: MigrationOptions): Promise<{
  candidates: Map<string, Set<string>>;
  sources: SourceStatus[];
}> {
  ...
  for (const scanner of [join(home, '.mnemonik', 'scanner.json'), join(state, 'scanner.json')]) {
    try {
      const parsed = JSON.parse(await readFile(scanner, 'utf8')) as Record<string, unknown>;
      ...
      for (const [field, label] of [
        ...
        if (Array.isArray(parsed[field]))
          for (const root of parsed[field])
            if (typeof root === 'string') addCandidate(candidates, root, label);
    } catch (error) {
      sources.push({
        name: 'scanner-state',
        path: scanner,
  ...
  for (const [name, path] of configs) await addConfig(candidates, sources, name, path);
  ...
  for (const [name, directory] of [
    ...
    for (const entry of entries) {
      ...
      if (decoded) addCandidate(candidates, decoded, name);
    }
  }
  ...
  for (const directory of workspaces) {
    try {
      const workspace = JSON.parse(
        await readFile(join(cursorData, directory, 'workspace.json'), 'utf8')
      ) as { folder?: unknown };
...
function classify(
  ...
  'kind' | 'path' | 'projectPath' | 'sources' | 'reachable' | 'sha256'
> {
  ...
  if (
    ...
    const droppedKeys = Object.keys(parsed).filter(
      (key) => !['projectId', 'projectName'].includes(key)
    );
    return {
      state: 'v0',
...
async function inspectProject(
  projectPath: string,
  sources: string[]
): Promise<IdentityInventoryEntry[]> {
    ...
    return [
      {
        ...
        state: 'unreachable',
        sha256: null,
        detail: `${errorCode(error) ?? 'ERROR'}: ${(error as Error).message}`,
      },
    ];
  }

  let identity: IdentityInventoryEntry;
  try {
...
export async function inventoryIdentityFiles(options: MigrationOptions): Promise<IdentityReport> {
  const home = options.home ?? homedir();
  const state = options.stateDir ?? stateDirectory(options.platform, process.env, home);
  const collected = await collectCandidates(options);
  const entries = (
    await Promise.all(
      [...collected.candidates].map(([path, source]) => inspectProject(path, [...source].sort()))
    )
  )
    .flat()
    .map((entry) => ({
  ...
  ) as Record<IdentityState, number>;
  return {
    schemaVersion: 1,
...
async function loadIndex(
  state: string,
  runId?: string
): Promise<{ index: BackupIndex; path: string }> {
  ...
  if (!id)
    for (const candidate of (await files(parent)).filter(isRunId).sort().reverse())
      if (
        await access(join(parent, candidate, 'index.json')).then(
          () => true,
          () => false
        )
      ) {
  ...
  const index = JSON.parse(await readFile(path, 'utf8')) as BackupIndex;
...
async function readBackup(indexPath: string, entry: BackupEntry): Promise<Buffer> {
...
export async function runIdentityMigration(options: MigrationOptions): Promise<MigrationResult> {
  const state =
    options.stateDir ?? stateDirectory(options.platform, process.env, options.home ?? homedir());
  ...
  if (options.mode === 'backup') {
    ...
    for (const item of report.entries.filter(
      (entry) => entry.kind === 'identity' && entry.state === 'v0'
    )) {
    ...
    const index: BackupIndex = { schemaVersion: 1, runId, createdAt: report.generatedAt, entries };
    ...
    for (const entry of loaded.index.entries) {
      try {
        ...
        const next = Buffer.from(
          `${JSON.stringify({ schemaVersion: 1, projectId: entry.projectId, ...(entry.projectName === undefined ? {} : { projectName: entry.projectName }) }, null, 2)}\n`
        );
  ...
  return {
    status:
      options.mode === 'apply' ? 'applied' : options.mode === 'verify' ? 'verified' : 'rolled_back',
