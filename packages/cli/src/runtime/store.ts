...
export class RuntimeStore extends RuntimeReader {
      ...
      try {
        for (const [name, bytes] of Object.entries(source.files)) {
          ...
          const fd = await open(
            path,
            'wx',
            source.manifest.files[name]?.executable ? 0o700 : 0o600
          );
          try {
        ...
        if (known) {
          ...
          if (
            JSON.stringify(existing.manifest.files) !== JSON.stringify(source.manifest.files) ||
        ...
        if (process.platform !== 'win32') {
          const sync = async (dir: string): Promise<void> => {
            for (const entry of await readdir(dir, { withFileTypes: true }))
              if (entry.isDirectory()) await sync(join(dir, entry.name));
        ...
        await this.verifyAt(artifact, ref, target);
        if (!activate) return this.verifyAt(artifact, ref, target);
  ...
  async rollbackRuntime(artifact: Artifact, expectedCurrent?: Reference): Promise<Verified> {
    await this.inspect(dirname(this.pointerPath(artifact)), true);
    const { atomicWrite, withLock } = await import('@mnemonik/local-setup');
    return withLock(
      join(dirname(this.pointerPath(artifact)), 'mutation'),
      5000,
      async (assertOwned) => {
        ...
        await this.verifyAt(
          artifact,
          p.previous,
          join(dirname(this.pointerPath(artifact)), p.previous.version)
        );
        await atomicWrite(
          this.pointerPath(artifact),
...
export async function updateRuntime(update: RuntimeUpdate): Promise<Verified> {
...
export async function hostNpmSource(
  ...
  const exactBytes = async (address: string, name: string, version: string) => {
    try {
  ...
  for (const expected of pin.closure) {
    ...
    ) as { name: string; version: string; dist: { integrity: string; tarball: string } };
    ...
    const pkg = JSON.parse(contents['package.json']?.toString() ?? '{}') as {
      name: string;
      version: string;
