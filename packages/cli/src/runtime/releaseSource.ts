...
export function devReleaseActive(): boolean {
  if (process.env.MNEMONIK_DEV_RELEASE_DIR) return true;
  try {
    return (
      JSON.parse(readFileSync(join(stateDirectory(), 'host-ownership.json'), 'utf8'))
...
): ReadinessDocument & { devReleaseSource?: true } {
  if (!devReleaseActive()) return document;
  return {
    ...document,
    devReleaseSource: true,
    installation: {
...
async function devBytes(url: URL): Promise<Buffer | undefined> {
  const directory = process.env.MNEMONIK_DEV_RELEASE_DIR;
  if (!directory) return undefined;
  const local = async (name: string) => {
    if (!/^[a-zA-Z0-9._-]+$/.test(name)) throw new RuntimeError('permission');
    const root = await realpath(resolve(directory));
    const path = join(root, name);
    if ((await realpath(path)) !== path) throw new RuntimeError('permission');
    return readFile(path);
  };
  if (url.href.startsWith(releaseRoot)) return local(url.pathname.split('/').at(-1) ?? '');
  if (url.origin !== 'https://registry.npmjs.org') return undefined;
  const parts = url.pathname.slice(1).split('/').map(decodeURIComponent);
  if (parts[0] === '@mnemonik' && parts[1] === '-') return local(parts[2] ?? '');
  const index = JSON.parse(
    (
      await local('index.json').catch((error: NodeJS.ErrnoException) => {
        if (error.code === 'ENOENT') return Buffer.from('{}');
        throw error;
      })
    ).toString()
  ) as Record<string, { version: string; tarball: string; integrity: string }>;
  const pinned = index[parts[0] ?? ''];
  if (pinned) {
    if (!/^\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(pinned.version)) throw new RuntimeError('permission');
  ...
  if (!parts[0]?.startsWith('@mnemonik/')) return undefined;
...
export async function scannerReleaseSource(
  ...
  if (hash(bytes) !== trusted.digestsSha256) throw new RuntimeError('digest_mismatch');
  const index = JSON.parse(bytes.toString()) as {
...
export async function npmReleaseSource(
  fetcher: Fetch = fetch,
  view: () => Promise<NpmDist> = async () => {
    ...
    const { stdout } = await promisify(execFile)(
      ...
      process.platform === 'win32' ? [windowsNpm, ...args] : args,
      { env: { ...process.env, NPM_CONFIG_IGNORE_SCRIPTS: 'true' } }
    );
    return JSON.parse(stdout) as NpmDist;
  }
): Promise<RuntimeSource> {
  ...
  if (
    pkg.name !== '@mnemonik/cli' ||
    pkg.version !== dist.version ||
    Object.keys(pkg.dependencies ?? {}).length ||
    ['preinstall', 'install', 'postinstall', 'prepare'].some((key) =>
  ...
  const files = Object.fromEntries(
    Object.entries(contents).map(([name, bytes]) => [prefix + name, bytes])
  );
  return {
    files,
    manifest: {
  ...
  const trusted = JSON.parse(
    await readFile(
      process.env.MNEMONIK_DEV_RELEASE_DIR
        ...
        : new URL('../scanner-release.json', import.meta.url),
      'utf8'
    )
  ) as ReleaseManifest;
