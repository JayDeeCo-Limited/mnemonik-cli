...
const bootstrapFrames = ['|', '/', '-', '\\'] as const;

export function bootstrapProgress(
  stream: BootstrapWritable = process.stdout,
  interactive = Boolean(process.stdout.isTTY),
  inherited = false
): { stop(): void } {
  if (!interactive) {
    if (!inherited) stream.write('Preparing the installer\n');
...
function bootstrapStateDirectory(
...
export async function guardNpmLaunch(entry: string): Promise<NpmLaunch> {
  const absolute = resolve(entry);
  const root = dirname(dirname(absolute));
  if (
    !root.replaceAll('\\', '/').endsWith('/' + cliKey) ||
  ...
  const pkg = await json<Package>(join(root, 'package.json'));
  const lock = await json<{ packages: Record<string, LockEntry> }>(
...
export function unpack(tarball: Buffer): Record<string, Buffer> {
  ...
  for (let offset = 0; offset + 512 <= tar.length;) {
    ...
    if (
      checksum !== parseInt(field(148, 8).trim(), 8) ||
    ...
    if (kind !== '5') {
      ...
      if (Object.hasOwn(files, key)) throw new RuntimeError('permission');
...
async function tarballBytes(entry: LockEntry, prefix: string): Promise<Buffer> {
  let bytes: Buffer;
  if (entry.resolved.startsWith('file:'))
    bytes = await readFile(
      fileURLToPath(new URL(entry.resolved.slice(5), pathToFileURL(prefix + '/')))
    );
  else {
    if (!entry.resolved.startsWith('https://')) throw new RuntimeError('unsigned');
...
export async function npmSource(launch: NpmLaunch): Promise<RuntimeSource> {
  const files: Record<string, Buffer> = Object.create(null) as Record<string, Buffer>;
  ...
  while (queue.length) {
    ...
    if (!pinned || pinned.link) throw new RuntimeError('unsigned');
    ...
    if (!packageBytes) throw new RuntimeError('manifest_missing');
    const pkg = JSON.parse(packageBytes.toString()) as Package;
    if (
    ...
    for (const [name, bytes] of Object.entries(contents)) {
      ...
      if (!(await lstat(path)).isFile() || (await realpath(path)) !== path)
        throw new RuntimeError('permission');
      if (!bytes.equals(await readFile(path))) throw new RuntimeError('digest_mismatch');
    }
    for (const dependency of Object.keys(pkg.dependencies ?? {})) {
      let parent = key;
      while (!launch.packages[(parent ? parent + '/' : '') + 'node_modules/' + dependency]) {
        if (!parent) throw new RuntimeError('manifest_missing');
...
export async function installBootstrap(
  ...
  const installed = async (): Promise<Record<string, string> | undefined> => {
    try {
    ...
    return json<Record<string, string>>(join(root, 'bootstrap-digests.json'));
  };
  ...
  for (const name of ['runtimeReader.js', 'runtimeSigners.js']) {
    const bytes =
      source.files[`node_modules/@mnemonik/shared/dist/${name}`] ??
      source.files[`${cliKey}/dist/vendor/shared/${name}`];
    if (!bytes) throw new RuntimeError('manifest_missing');
    files[`node_modules/@mnemonik/shared/dist/${name}`] = bytes;
  }
  files['node_modules/@mnemonik/shared/package.json'] = Buffer.from(
    JSON.stringify({
      type: 'module',
  ...
  files['package.json'] = Buffer.from(
    JSON.stringify({ type: 'module', name: '@mnemonik/runtime-bootstrap' })
  );
  const bootstrapFiles = Object.fromEntries(
    Object.entries(files).map(([name, bytes]) => [name, hash(bytes)])
  );
  const matches = (digests: Record<string, string> | undefined): boolean =>
    !!digests &&
  ...
  return store.withMutationLock('cli', async (assertOwned) => {
    ...
    try {
      ...
      const staged = JSON.parse(
        (await store.bytes(join(stage, 'bootstrap-digests.json'))).toString()
      ) as Record<string, string>;
      if (!matches(staged)) throw new RuntimeError('digest_mismatch');
      ...
      try {
        ...
        if (current) await rename(previous, root);
        throw e;
      }
    } catch (e) {
...
export function newerVersion(candidate: string, current: string): boolean {
  ...
  for (let i = 0; i < Math.max(a.length, b.length); i++) {
    ...
    if (left === undefined) return i === 3;
    if (right === undefined) return i !== 3;
...
export async function bootstrap(args: string[], entry = process.argv[1] ?? ''): Promise<number> {
  ...
  const progress =
    args[0] === 'install'
      ...
      : undefined;
  return bootstrapWithProgress(args, entry, progress).catch((error: unknown) => {
    progress?.stop();
    throw error;
  });
}
...
async function bootstrapWithProgress(
  args: string[],
  entry: string,
  progress: ReturnType<typeof bootstrapProgress> | undefined
): Promise<number> {
  const root = fileURLToPath(new URL('../../', import.meta.url)).replace(/[\\/]$/, '');
  if ((await json<Package>(join(root, 'package.json'))).name === '@mnemonik/runtime-bootstrap') {
    const store = new RuntimeStore(dirname(dirname(root)));
    if (root !== join(store.state, 'runtimes', 'bootstrap')) throw new RuntimeError('permission');
    await store.inspect(join(root, 'dist', 'bin.js'));
    if (args[0] === '--runtime-pointer') {
      if (args[1] !== store.pointerPath('scanner')) throw new RuntimeError('permission');
    ...
    const { runCli } = (await import(
      pathToFileURL(verified.entry).href
    )) as typeof import('../router.js');
    progress?.stop();
    return runCli(args);
  }
  ...
  if (launch.root !== root) throw new RuntimeError('permission');
  const source = await npmSource(launch);
  const store = new RuntimeStore(bootstrapStateDirectory());
  const current = await readFile(store.pointerPath('cli')).then(
    () => store.verifyRuntime('cli'),
  ...
  return launchChild(
    process.execPath,
    [launcher, ...args],
    progress ? { MNEMONIK_BOOTSTRAP_PROGRESS: '1' } : undefined
  );
}

const children = new Set<ChildProcess>();
const interruptChildren = (): void => {
  for (const child of children) child.kill('SIGINT');
};
const terminateChildren = (): void => {
  for (const child of children) child.kill('SIGTERM');
};
const hangupChildren = (): void => {
  for (const child of children) child.kill('SIGHUP');
};
