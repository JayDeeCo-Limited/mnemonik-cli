...
import { reviewScannerProjects, type ScannerPickerResult } from '../scanner/picker.js';
import { hostOrder, type HostAdapter, type HostName, type Target } from './adapters.js';
import {
  bytesAt,
  withInstall,
  type Consent,
...
export interface InstallDependencies {
  ...
  services?: {
    ...
    inspect(): Promise<Array<{ id: string; before: string }>>;
    start(
      id: string
    ): Promise<{ started: true; alreadyRunning: false } | { started: false; alreadyRunning: true }>;
    restore(id: string, before: string): Promise<void>;
  };
  upload?: {
    /** The server uses this operation ID as its idempotency key. */
    start(operationId: string): Promise<{ uploaded: boolean; deduplicated: boolean }>;
...
export const componentRevoker =
  (adapter: ReturnType<typeof createCredentialAdapter>, transport: CredentialTransport) =>
  async (reference: string) =>
    (await adapter.revokeFamily(reference, transport)).status === 'revoked';
export async function revokeInstallComponent(
  stateDir: string,
  reference: string,
  fetcher: typeof fetch = fetch
) {
  const credentials = createCliCredentials({ stateDir });
  if (!(await credentials.readFamily(reference))) return true;
  return componentRevoker(credentials, {
    ...
    revokeFamily: async (id, token) => {
      const response = await fetcher(
        `${apiOrigin()}/api/v1/component-credentials/${encodeURIComponent(id)}/revoke`,
        {
          method: 'POST',
...
export const consentMatches = (a: Consent | undefined, b: Consent): boolean =>
  a !== undefined &&
  a.account === b.account &&
...
const required = <T>(value: T | undefined): T => {
  if (value === undefined) throw new Error('Incomplete install journal');
  return value;
};
const adapterFor = (deps: InstallDependencies, host: HostName) => {
  const adapter = deps.adapters.find((a) => a.name === host);
  if (!adapter) throw new Error(`Host adapter unavailable: ${host}`);
  return adapter;
};
...
async function retainProjects(journal: Journal, deps: InstallDependencies) {
  for (const project of journal.data.projects) {
    ...
    if (bytes) {
      const record = JSON.parse(bytes.toString()) as SetupRecord;
      project.uuid ??= record.remote?.projectId;
      if (record.steps.remote.outcome !== 'linked') project.effect ??= record.steps.remote.outcome;
    }
    if (project.uuid) {
      project.empty = await deps.projectEmpty?.(project.uuid);
      report(
        journal,
        `Remote project ${project.uuid} retained${project.effect === 'restored' ? ' active; explicitly re-archive it in the console' : project.effect === 'created' && project.empty === true ? '; empty: you may explicitly archive it in the console' : '; no archive action offered'}.`
      );
    }
  }
  await journal.save();
...
export async function compensate(journal: Journal, deps: InstallDependencies, keepCli = false) {
  ...
  for (const project of [...journal.data.projects].reverse()) {
    try {
      ...
      if (journal.data.joined) continue;
      ...
      if (result.status === 'rolled_back') project.uuid ??= result.retainedRemoteUUID;
  ...
  if (journal.data.mutations.some((m) => m.event === 'upload_intent'))
    report(
      journal,
      `Uploaded data is retained. ${deps.upload?.deletionAction ?? 'Delete the cloud index separately in the console.'}`
    );
...
export async function reconcile(journal: Journal, deps: InstallDependencies) {
...
export async function runInstall(deps: InstallDependencies, resume?: Journal) {
  if (!deps.services && deps.input.components.includes('scanner'))
    deps = {
      ...
      services: scannerService({
        ...
        timeout: async (phase) =>
          (await deps.ui.timeout(`scanner ${phase}`)) === 'retry' ? 'retry' : 'skip',
        ...deps.scannerService,
      }),
    };
  return withInstall(
    ...
    resume,
    async (journal) => {
      const event = (name: MutationKind, target?: string) => journal.event(name, target);
      const cancelled = () => {
        if (deps.signal?.aborted) throw new Error('install_cancelled');
      };
      const cancel = async () => {
      ...
      try {
        ...
        for (const host of hostOrder.filter((h) => journal.data.hosts.includes(h))) {
          ...
          if (!detected.supported) {
            report(
              journal,
              `${host}: ${detected.reason ?? 'upgrade to a supported version and retry'}.`
            );
        ...
        while (true) {
          ...
          if (!consentMatches(journal.data.consent, consent)) {
            ...
            await journal.save();
            if (!(await deps.ui.consent(consent))) return cancel();
          ...
          for (const project of journal.data.projects) {
            project.selected = picked.repositories.some(
              (r) => r.selected && r.path === project.root
            );
            if (!project.selected) {
              ...
              await journal.restore(
                required(
                  journal.data.targets.find((t) => t.path === join(project.root, '.mnemonik.json'))
                )
              );
            }
          }
          ...
          for (const entry of handoff.staged) {
            ...
            const saved = await bytesAt(
              recordPath(entry.path, deps.projectStateDir ?? deps.stateDir)
            );
            const record = saved ? (JSON.parse(saved.toString()) as SetupRecord) : undefined;
            const target = required(
              journal.data.targets.find((t) => t.path === join(entry.path, '.mnemonik.json'))
            );
        ...
        for (const project of journal.data.projects.filter((p) => p.selected !== false)) {
          ...
          if (result.status !== 'done') throw new Error('Project Apply needs attention');
          const target = required(
            journal.data.targets.find((t) => t.path === join(project.root, '.mnemonik.json'))
          );
        ...
        if (deps.services) {
          for (const service of journal.data.services) {
            if (
              journal.data.mutations.some(
                (m) => m.event === 'service_started' && m.target === service.id
              )
            )
              continue;
        ...
        if (
          ...
          if (!journal.data.mutations.some((mutation) => mutation.event === 'upload_finished')) {
            let operationId = journal.data.mutations.find(
              (mutation) => mutation.event === 'upload_intent' && mutation.target
            )?.target;
            if (!operationId) {
        ...
        if (journal.data.hosts.length) report(journal, 'hook_not_verified');
        ...
        if (journal.data.phase === 'uploading')
          report(
            journal,
            `Upload failed; remote data is retained. ${required(deps.upload).deletionAction}`
          );
        else
          report(
            journal,
            `Installation interrupted (${installFailureReason(error)}); resume or roll back the recorded changes.`
          );
        await journal.save();
        if (deps.fault) throw error;
      }
      return journal.data;
    },
    deps.fault
