...
export async function joinedInstall(
  ...
  const hasLaunchHost = (await ownership.readOwnership(stateDir)).targets.some((target) =>
    launchHosts.includes(target.host as never)
  );
  ...
  const needsDetectedEditor =
    !previous && !automatic && !flags.has('hosts') && !flags.has('components') && !indexingOnly;
  ...
  try {
    ...
    if (indexingOnly) {
      ...
      if (selected === 'Cancel' || selected === 'Back') return 130;
      if (selected) {
      ...
      const chosen = [
        ...names.map((name) => labels[name as keyof typeof labels]),
        ...(scanner ? ['automatic project indexing'] : []),
      ];
      if (chosen.length) output.line(completedLine(chosen.join(', ')));
    }
    ...
    const afterHosts = async (
          ...
          const selectedRepositories = await Promise.all(
            scannerPlan.roots.map(async (selectedRoot) => {
              ...
              return {
                ...
                resolution,
                linkProjectId:
                  resolution?.kind === 'ok' ? resolution.identity.projectId : undefined,
              };
            })
          );
          ...
          for (const candidates of rootsByProject.values()) {
            ...
            const ranked = await Promise.all(
              candidates.map(async (candidate) => {
                ...
                const stored =
                  resolution?.kind === 'ok' ? resolution.identity.repositoryFingerprint : undefined;
            ...
            ranked.sort(
              (left, right) =>
                Number(right.fingerprintMatches) - Number(left.fingerprintMatches) ||
      ...
      if (!automatic) {
        ...
        if (!scannerInstallFailed) startProgress('Finishing installation');
      }
    };
    ...
    const configuredTargets =
      indexingOnly || editorChoiceOffered || names.length
        ? []
    ...
    if (json)
      ...
      activeProgress?.stop();
      activeProgress = undefined;
      if (!automatic) output.installSection();
