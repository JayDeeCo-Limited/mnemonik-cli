...
export interface ConsentDecision {
  ...
  answers: Array<{ answer: string; add: string[]; remove?: string[] }>;
}

const decisions: Record<string, ConsentDecision> = {
  'accept-indexing': {
...
const enableIndexing: ConsentDecision = {
...
export function consentDecision(
  flag: string,
  command: 'install' | 'other' = 'other',
  found: FoundEditor[] = []
): ConsentDecision | undefined {
  const name = flag.replace(/^--/u, '');
  if (name === 'accept-indexing' && command !== 'install') return enableIndexing;
...
export function missingConsent(
  ...
  else for (const line of consentLines(name, command, found)) output.error(line);
  return 3;
}
