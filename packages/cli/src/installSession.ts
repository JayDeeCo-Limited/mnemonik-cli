...
export async function reportInstall(
  ...
  if (json) output.json(document);
    ...
    for (const step of steps) {
      ...
      output.line(
        `  ${step.name}: ${step.status === 'READY' ? 'Done.' : humanReason(step.status)}${owner}${step.action ? ` - ${humanReport(step.action)}` : ''}`
      );
    }
    if (installSession.status !== 'completed') output.line(humanReason(installSession.status));
    output.line();
