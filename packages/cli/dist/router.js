...
export const alreadyConnectedFolderLine = (name) => `  ✓ ${name} is already connected and watched.`;
export const removedFolderLine = (name) => `  ✓ ${name} is no longer connected.`;
export const projectDeletionWarning = (name) => `Deleting ${name} removes its memories, code index and summaries for everyone. This cannot be undone.`;
export const projectDeletedLine = (name) => `Deleted ${name}.`;
export const NOTHING_DELETED_LINE = 'Nothing was deleted.';
export const NOTHING_REMOVED_LINE = 'Nothing was removed.';
export const SCANNER_UNINSTALL_PROMPT = 'Uninstalling background indexing means your code on this machine is no longer\n' +
...
async function revokeScannerCredential(deps, output, stateDir, saved) {
    const state = saved ??
        JSON.parse(await readFile(`${stateDir}/scanner/state.json`, 'utf8'));
    const bearer = await ensureCliAuth(deps, output, false);
    const response = await (deps.grantFetch ?? fetch)(`${apiOrigin()}/api/v1/component-credentials/${encodeURIComponent(state.config.credentialFamilyId)}/revoke`, { method: 'POST', headers: { Authorization: `Bearer ${bearer}` } });
    if (!response.ok)
...
async function uninstallScannerOptOut(deps, output, service, json) {
    ...
    const saved = await readFile(`${stateDir}/scanner/state.json`, 'utf8').then((text) => JSON.parse(text), (error) => {
    ...
    if (json)
        output.json({ status: 'uninstalled', consent: saved ? 'withdrawn' : 'none' });
    else
        output.line(SCANNER_REMOVED_LINE);
    return 0;
}
export const dataDeletePrompt = (projectId) => `This deletes everything background indexing has sent for ${projectId} from your account. Type yes to continue.`;
export const stillWatchedLine = (root) => `The folder is still being indexed. Run mnemonik remove ${root} to stop that.`;
export const identityFileKeptLine = "This folder's .mnemonik.json still points at the deleted project. Connecting the folder again creates a new project.";
...
function commandPath(positionals) {
    for (let length = positionals.length; length > 0; length--)
        if (helpScreen(positionals.slice(0, length)))
            return positionals.slice(0, length);
    return [];
}
/**
...
function usageFailure(output, parsed, failure, human) {
    if (parsed.json) {
        const command = failure.reason === 'unknown_command'
        ...
        output.json({
            ...
            action: failure.reason === 'unknown_command' || !command
                ? 'mnemonik --help'
...
function flagError(output, parsed, message) {
    const [, kind, detail] = /^(Unknown flag|Missing value for flag)(?: combination)?: (\S+)/u.exec(message) ?? [undefined, 'Unknown flag'];
...
async function ensureCliAuth(deps, output, noBrowser, showIdentity = true, fresh = false) {
    ...
    let bearer = fresh
        ...
        : await cliAuth.getCliBearer().catch((error) => {
...
async function runHostCommand(command, parsed, deps, output, scannerSelected = false) {
    ...
    if ((host && !hostOrder.includes(host)) ||
        ...
        return usageFailure(output, parsed, {
    ...
    if (command === 'install') {
        ...
        const unknownHost = names.find((name) => !hostOrder.includes(name));
        if (unknownHost !== undefined)
            return usageFailure(output, parsed, { reason: 'invalid_value', detail: unknownHost }, 'Invalid hosts');
...
async function deleteProjectCommand(parsed, rest, deps, output) {
    ...
    if (!target) {
        output.error('This folder is not connected to a project. Run mnemonik project delete <name> instead.');
        return 3;
    }
    ...
    if (!listed?.ok) {
        ...
        const reason = !listed
            ...
            : listed.status === 401 || listed.status === 403
                ...
                : 'server_error';
        if (json)
            output.json({ status: 'action_required', reason, action: accountActions[reason] });
        else
            output.error(humanReason(reason));
        return 3;
    }
    ...
    const project = projects.find((candidate) => candidate.id === target || candidate.name?.toLowerCase() === target.toLowerCase());
    if (!project) {
    ...
    if (typeof supplied !== 'string') {
        if (parsed.flags.has('non-interactive') || json) {
            output.error(`To skip this check, run mnemonik project delete ${project.name} --confirm "${project.name}".`);
            return 3;
        }
    ...
    if (!response.ok) {
        output.error(response.status === 403
    ...
    if (json)
        ...
        if (stillWatched)
            output.line(stillWatchedLine(root));
        if (hereId === project.id)
            output.line(identityFileKeptLine);
    }
    return 0;
}
export async function runCli(args, deps = {}) {
    const parsed = parse(args);
    const helpIndex = args.findIndex((argument) => argument === '--help' || argument.startsWith('--help='));
    ...
    const usageError = (positionals, failure = usageReason(parsed.positionals, positionals)) => usageFailure(output, parsed, { path: positionals, ...failure }, () => stderr.write(helpScreen(positionals) ?? help));
    /** Too few or too many arguments after the command's words. */
    const argumentError = (screen, words, given, most) => usageError(screen, given.length > most
    ...
    if (parsed.error)
        return flagError(output, parsed, parsed.error);
    ...
    if (command === 'roots' || command === 'add' || command === 'remove') {
        ...
        if (invalid ||
            ...
            return invalid
                ...
                : !['add', 'remove', 'list'].includes(action ?? '')
                    ? usageError([command])
                    : argumentError([command], command === 'roots' ? [command, action ?? ''] : [command], actionArguments.map(String), action === 'list' ? 0 : 1);
        const stateDir = deps.installStateDir ?? stateDirectory(process.platform, process.env, deps.home);
        const saved = JSON.parse(await readFile(`${stateDir}/scanner/state.json`, 'utf8').catch(() => 'null'));
        if (!saved)
    ...
    if (command === 'auth' &&
        ...
        try {
            await revokeScannerCredential(deps, output, deps.installStateDir ?? stateDirectory(process.platform, process.env, deps.home));
            const result = {
                status: 'revoked',
                verbs: ['revoke access'],
    ...
    if ((command === 'update' || command === 'uninstall') &&
        ...
        try {
            ...
            if (command === 'update') {
                ...
                return await uninstallScannerOptOut(deps, output, scannerService(options), parsed.flags.has('json'));
            return 0;
        }
        catch (error) {
        ...
        if (invalid ||
            ...
            return invalid
                ...
                : subcommand === 'preview' || subcommand === 'send'
                    ? argumentError(['diagnostics'], ['diagnostics', subcommand], rest, subcommand === 'send' ? 1 : 0)
    ...
    if (command === 'connect') {
        ...
        if (!subcommand || rest.length || !hostOrder.includes(subcommand))
            return subcommand && !hostOrder.includes(subcommand)
    ...
    if (command === 'project') {
        ...
        if (subcommand === 'ensure') {
            ...
            if (rest.length)
                return usageError(['project', 'ensure']);
        ...
        if ((subcommand === 'link' && (rest.length < 1 || rest.length > 2)) ||
            ...
            return argumentError(['project', subcommand], ['project', subcommand], rest, subcommand === 'link' ? 2 : 1);
    ...
    if (command === 'identity') {
        if (subcommand !== 'migrate')
            return usageError(['identity', 'migrate']);
        ...
        if (selected.length > 1)
            return usageFailure(output, parsed, { reason: 'invalid_flag', detail: `--${selected[1]}` }, 'Choose one migration phase: --report, --backup, --apply, --verify, or --rollback');
        const mode = (selected[0] ?? 'report');
        if (rest.length && mode !== 'report' && mode !== 'backup')
            return usageFailure(output, parsed, { reason: 'invalid_value', detail: rest[0] }, 'Paths are only accepted by --report and --backup');
        let result;
        try {
    ...
    if (command === 'auth') {
        ...
        if (invalid ||
            ...
            return invalid
                ? flagError(output, parsed, invalid)
                : usageError(subcommand && helpScreen(['auth', subcommand]) ? ['auth', subcommand] : ['auth'], !rest.length && ['login', 'renew', 'status', 'logout'].includes(subcommand ?? '')
                    ? { reason: 'invalid_flag', detail: '--reopen-install' }
                    : undefined);
        const host = parsed.flags.get('host');
        if (host && !hostOrder.includes(host))
            return usageFailure(output, parsed, { reason: 'invalid_value', detail: String(host) }, 'Invalid host');
        ...
        if (subcommand === 'login' || subcommand === 'renew') {
            if (host)
                return usageFailure(output, parsed, { reason: 'invalid_flag', detail: '--host' }, 'Use mnemonik connect <host> for a host login');
