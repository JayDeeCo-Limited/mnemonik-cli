...
const within = (path, parent, platform) => {
...
async function canonical(path, platform) {
...
export async function evaluateRoot(resolution, options) {
    ...
    if (root === paths.parse(root).root)
        return { allowed: false, root, reason: 'filesystem_root' };
    if (root === home)
        return { allowed: false, root, reason: 'home_directory' };
    if (root === temporary)
        return { allowed: false, root, reason: 'temporary_directory' };
    ...
    if (['.claude', '.codex', '.cursor', '.grok', '.copilot', '.mnemonik'].some((name) => within(root, paths.join(home, name), platform)))
