...
export function humanReport(report) {
    const malformedHost = /^(claude-code|codex|cursor): (?:invalid_json|invalid_toml|foreign_schema)\.$/u.exec(report)?.[1];
    if (malformedHost) {
        const name = malformedHost === 'claude-code'
            ? 'Claude Code'
            : malformedHost === 'codex'
                ? 'Codex'
                : 'Cursor';
    ...
    const code = /(?:^|[:(]\s*)([a-z][a-z0-9]*(?:_[a-z0-9]+)+|permission|unsigned|E[A-Z]+)(?=$|[.):\s])/u.exec(report)?.[1];
    return code
        ? humanReason(code)
...
//# sourceMappingURL=humanReason.js.map