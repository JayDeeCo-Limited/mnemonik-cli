...
export async function controlScanner(action, options) {
    ...
    return withLock(join(options.stateDir, 'scanner/control'), 5000, async () => {
        const id = randomUUID();
        await atomicWrite(join(options.stateDir, 'scanner/control.json'), Buffer.from(JSON.stringify({ id, action })));
        ...
        do {
            ...
            if (receipt?.snapshot.lifecycle.pid === supervisor.pid &&
