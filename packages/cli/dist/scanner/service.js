...
export class ScannerServiceLimited extends Error {
    ...
    get action() {
        if (this.reason.startsWith('scanner_replacement_') &&
...
export function scannerService(options) {
    ...
    const command = options.command ??
        (async (operation, definition) => {
            ...
            const fixed = !replacementRuntime && (options.platform ?? process.platform) === 'darwin'
                ? await lstat(macScannerLauncher).catch((error) => {
                    if (error.code === 'ENOENT')
                        return null;
                    throw error;
                })
            ...
            return new Promise((resolve, reject) => {
                const child = execFile(entry, ['service', operation, '--json'], {
                    timeout: (options.platform ?? process.platform) === 'darwin'
                        ? SCANNER_MAC_COMMAND_BUDGET_MS
    ...
    const locallyReady = async (since, pid, paused = false) => scannerAttemptHealthy(await scannerReceipt(options.stateDir).catch(() => null), { pid, startedAt: since }, paused);
    ...
    return {
        ...
        async recover() {
            if ((options.platform ?? process.platform) !== 'darwin')
                return false;
            return withLock(join(options.stateDir, 'scanner/launchd-replacement-control'), 5000, async () => {
                const receipt = await scannerReceipt(options.stateDir);
                const snapshot = receipt?.snapshot;
                const attempt = await readFile(join(options.stateDir, 'scanner/service-replacement/result.json'), 'utf8')
                    .then((text) => JSON.parse(text))
                    .catch(() => null);
                const age = now() - Number(attempt?.startedAt);
                if (attempt?.pid && age <= SCANNER_HANDOFF_BUDGET_MS)
                    return false;
                const stalledStart = attempt?.pid && Number.isFinite(age) && age > SCANNER_HANDOFF_BUDGET_MS;
                const staleReceipt = snapshot &&
                    receipt &&
                ...
                if (!pid || (!stalledStart && !staleReceipt))
                    return false;
                ...
                if (service.binaryPath !== runtime.entry) {
                    const pointer = JSON.parse((await store.bytes(store.pointerPath('scanner'))).toString());
                    if (!pointer.previous)
                        return false;
                ...
                if (service.kind !== 'launchd' ||
                    ...
                    !process.getuid)
                    return false;
                if (!(options.pidIdentity ?? pidIsScanner)(pid, 'darwin', undefined, {
        ...
        async start(_id = 'scanner', heartbeatAfter) {
            ...
            await waitFor('heartbeat', SCANNER_STARTUP_BUDGET_MS, () => heartbeat(heartbeatAfter ?? (alreadyRunning ? startedAt - 300000 : startedAt), state.pid));
            verified = true;
        ...
        async replace(runtime, replacement) {
            ...
            verified = false;
            const saved = JSON.parse(replacement.state?.after ??
            ...
            while (now() < deadline) {
                ...
                if (attempt && attempt.startedAt >= startedAt) {
                    ...
                    if (attempt.fallbackError)
                        throw new ScannerServiceLimited(attempt.fallbackError === 'scanner_fallback_missing'
                            ? 'scanner_replacement_candidate_failed'
                            : 'scanner_replacement_failed', [attempt.candidateError, attempt.fallbackError].filter(Boolean).join('; '));
                    const active = await invoke('status');
                    if (active.running &&
                        active.pid === attempt.pid &&
                        (await locallyReady(attempt.startedAt, active.pid, saved.paused))) {
                        if (attempt.fallback)
                            throw new ScannerServiceLimited('scanner_replacement_rolled_back', attempt.candidateError);
                        verified = true;
                        return;
                    }
                }
        ...
        async uninstall() {
            const native = (timeout) => options.supervisorRun ??
                (async (file, args) => (await promisify(execFile)(file, args, { encoding: 'utf8', timeout })).stdout);
            try {
                if ((options.platform ?? process.platform) === 'darwin') {
                    ...
                    return await withLock(join(options.stateDir, 'scanner/launchd-replacement-control'), SCANNER_REMOVAL_BUDGET_MS, async () => {
                        await removeMacScanner(options.stateDir, options.home ?? homedir(), options.uid ?? process.getuid?.() ?? -1, native(SCANNER_REMOVAL_BUDGET_MS), { now, sleep, plist: options.systemPlist });
                ...
                if ((options.platform ?? process.platform) === 'linux') {
                    const recordedPid = await readFile(join(options.stateDir, 'scanner/status.json'), 'utf8')
                        .then((text) => JSON.parse(text).snapshot
                        ?.lifecycle?.pid)
                        .catch((error) => {
                        if (error.code === 'ENOENT')
                            return undefined;
                    ...
                    await uninstallSystemdUnit(join(process.env.XDG_CONFIG_HOME ?? join(options.home ?? homedir(), '.config'), 'systemd/user/mnemonik-scanner.service'), native(SCANNER_STOP_BUDGET_MS), { now, sleep, recordedPid });
                }
                else {
                    await invoke('stop');
                ...
                throw new ScannerServiceLimited('scanner_stop_failed', [error.message, cause instanceof Error ? cause.message : undefined]
                    .filter(Boolean)
                    .join(': '));
            }
        },
        async restore(_id, before) {
            ...
            if (previous.installed && previous.definition) {
                ...
                if (previous.running)
                    await this.start('scanner', restoredAt);
                else
                    await invoke('stop');
                return;
            }
