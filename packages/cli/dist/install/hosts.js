...
export function codexTrustAction(_resolvedPath) {
...
export const CODEX_TRUST_MIGRATION = "Mnemonik's Codex hooks no longer change when your sign-in changes. Codex will ask you to trust them one last time.";
// A command written before L-86: the credential family sat inside the text Codex fingerprints.
const LEGACY_CODEX_COMMAND = / --credential-family [A-Za-z0-9_-]+(?: --server \S+)? --mnemonik-owner=codex-hooks/;
...
function targetFor(selection, runtime, store, owned) {
    return {
...
function environment(selection, env = process.env, owned = false) {
    const result = { ...env, HOME: selection.home, USERPROFILE: selection.home };
    if (owned)
        result.MNEMONIK_CLI_OWNED_TARGET = '1';
...
async function apply(journal, run) {
...
function actionFor(reason, selection, resolvedPath) {
    ...
    if (reason === 'codex_trust_pending')
        return codexTrustAction(resolvedPath);
    if (reason === 'codex_trust_declined')
        return 'Approve the Mnemonik hook in Codex.';
...
async function codexHooksDisabled(changes) {
    ...
    for (const line of raw.split(/\r?\n/)) {
        ...
        else if (features && /^\s*(?:hooks|"hooks"|'hooks')\s*=\s*false\b/.test(line))
            return true;
    }
    return false;
}
async function removeUnusedRuntimePointers(journal, store, stateDir, hosts) {
    const owned = await readOwnership(stateDir);
    for (const host of new Set(hosts)) {
        if (owned.targets.some((target) => target.host === host))
            continue;
        const pointer = store.pointerPath(host);
        if (!(await bytesAt(pointer)))
            continue;
...
const protectInstall = (deps, work) => async (journal) => {
    try {
        ...
        await journal.save();
        if (deps.rollbackInstall)
            await deps.rollbackInstall(journal);
        else
            journal.data.phase = (await journal.restoreFiles()) ? 'rolled_back' : 'rolling_back';
...
export async function runHosts(command, selections, deps, allowMigration = false) {
    ...
    const outcome = await withInstall(deps.stateDir, {
        ...
        ...deps.installPlan,
    }, pending, protectInstall(deps, async (journal) => {
        const results = [];
        ...
        if (pending?.data.joined) {
            ...
            for (const run of journal.data.hostRuns.filter((r) => r.status !== 'rolled_back' && !completed.includes(r))) {
                ...
                for (const target of journal.data.targets)
                    if (target.group === run.id)
                        target.group = restoredGroup;
            ...
            for (const run of completed) {
                ...
                for (const target of journal.data.targets)
                    if (target.group === run.id)
                        target.group = `superseded:${target.id}:${run.id}`;
            }
            ...
            const request = pending.data.hostRequest;
            if (!request)
                throw new Error('host_request_missing');
            const retry = request.selections.filter((_, index) => !['complete', 'verified'].includes(journal.data.hostRuns?.[index]?.status ?? 'pending'));
            ...
            if (request.command === 'uninstall')
                await removeUnusedRuntimePointers(journal, new RuntimeStore(deps.stateDir), deps.stateDir, request.selections.map((selection) => selection.host));
        ...
        for (const selection of selections) {
            ...
            let old = record.targets.find((t) => t.host === selection.host &&
            ...
            try {
                ...
                if (beforePointer)
                    await store.verifyRuntime(selection.host);
                let runtime;
                if (command === 'update' || command === 'install') {
                    ...
                    if (!beforePointer)
                        freshRuntimeVersion = runtime.reference.version;
                ...
                if (target.component === 'hooks') {
                    ...
                    if (command !== 'update' &&
                        ...
                        if (!/^[A-Za-z0-9_-]+$/.test(family.id))
                            throw new Error('invalid_credential_family');
                    ...
                    if (target.credentialFamily && command !== 'uninstall') {
                        const reference = await journal.plan(join(deps.stateDir, 'hook-families', `${hash(run.id)}.json`), Buffer.from(JSON.stringify({ credentialFamily: target.credentialFamily })), { kind: 'credential-reference', host: run.host, group: run.id });
                        await journal.stage(reference);
                        await journal.commit(reference);
                    }
                }
                const changes = [];
                let ownedChanges = changes;
                let profilePath;
                if (command === 'uninstall') {
                    ...
                    for (const file of old?.files ?? []) {
                        ...
                        if (digest(bytes) !== file.original.hash)
                    ...
                    if (plan.version !== runtime.manifest.version ||
                ...
                if (command !== 'uninstall') {
                    ...
                    old = record.targets.find((t) => t.host === selection.host &&
                        t.component === (selection.component ?? 'hooks') &&
                        t.scope === selection.scope &&
                        t.profilePath === selection.profilePath);
                    const id = old?.id ?? identity(selection);
                    for (const entry of journal.data.targets)
                        if (entry.group === run.id)
                            entry.group = id;
                    run.id = id;
                ...
                run.remove = record.targets
                    .filter((t) => t.host === selection.host &&
                    t.component === (selection.component ?? 'hooks') &&
                ...
                if (command !== 'uninstall') {
                    ...
                    const candidate = {
                        ...
                        files: ownedChanges.map((c) => {
                            ...
                            const planned = journal.data.targets.find((entry) => entry.group === run.id && entry.path === c.path);
                            return {
                                path: c.path,
                                hash: hash(c.content),
                ...
                if (deps.fault || (deps.afterHosts && error.code))
                    throw error;
        ...
        if (command === 'uninstall')
            await removeUnusedRuntimePointers(journal, store, deps.stateDir, selections.map((selection) => selection.host));
        journal.data.phase = 'complete';
        ...
        if (deps.afterHosts) {
            ...
            await deps.afterHosts(journal, results, async () => {
                for (const check of completionChecks.splice(0))
                    await check();
                journal.data.state = results.some((r) => r.status === 'ACTION_REQUIRED')
                    ? 'ACTION_REQUIRED'
                    : results.every((r) => r.status === 'READY')
    ...
    if (outcome.recovery?.selections.length) {
        const recovered = await runHosts(outcome.recovery.command, outcome.recovery.selections, deps, outcome.recovery.allowMigration);
        return {
            ...recovered,
...
export async function selectOwned(state, host, scope, component) {
    const selected = (await readOwnership(state)).targets.filter((t) => (!host || t.host === host) &&
        (!scope || t.scope === scope) &&
        (!component || t.component === component));
    const ambiguous = selected.filter((t) => selected.some((other) => other.id !== t.id &&
        other.host === t.host &&
        other.scope === t.scope &&
        other.component === t.component));
    return { selected, ambiguous: ambiguous.map((t) => t.profilePath) };
}
export async function codexTrustConditions(deps) {
    const store = new RuntimeStore(deps.stateDir);
    for (const owned of (await readOwnership(deps.stateDir)).targets) {
        if (owned.host !== 'codex' || owned.component !== 'hooks')
            continue;
        try {
...
export async function hookStatusConditions(deps, hosts) {
...
export async function revokeHostGrants(host, deps, grantId) {
    ...
    if (status.account !== deps.account)
        throw new Error('host_account_mismatch');
    const { grantHost } = await import('../auth/status.js');
    const grants = status.grants.filter((g) => (!grantId || g.id === grantId) && grantHost(g) === host && g.resource === `${apiOrigin()}/mcp`);
...
export async function logoutHost(host, deps) {
