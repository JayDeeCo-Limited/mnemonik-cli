...
export async function abandonInterrupted(state = stateDirectory()) {
    ...
    if (!pending || pending.data.hostRequest?.command === 'uninstall')
        return;
    await withInstall(state, pending.data, pending, async (journal) => {
        const latest = (await interrupted(state)).find((candidate) => candidate.data.runId === journal.data.runId);
        if (!latest)
            return;
 ...
 */
function untouchedScannerRun(data) {
    return ((data.phase === 'preparing' || data.phase === 'review') &&
        data.hosts.length === 0 &&
 ...
 */
export async function closeUntouchedScannerRun(state = stateDirectory()) {
    const pending = (await interrupted(state))[0];
    if (!pending || !untouchedScannerRun(pending.data))
        return false;
    let closed = false;
    await withInstall(state, pending.data, pending, async (journal) => {
        const latest = (await interrupted(state)).find((candidate) => candidate.data.runId === journal.data.runId);
        if (!latest || !untouchedScannerRun(latest.data))
            return;
        Object.assign(journal.data, latest.data);
        if (!journal.data.reports.includes('installation_abandoned'))
            journal.data.reports.push('installation_abandoned');
