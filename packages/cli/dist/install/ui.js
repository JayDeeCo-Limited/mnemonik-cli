...
import { cancelScreen, finalReviewScreen, interruptedScreen, renderScreen, } from '../screens.js';
...
export function terminalInstallUI(input, output, roots) {
    ...
    const ui = {
        waiting: (host) => output.line(`Waiting for ${host}, up to ${host === 'scanner heartbeat' ? '1 minute' : '2 minutes'}.`),
        timeout: async (host) => {
        ...
        recovery: async (reports) => (await ask({ ...interruptedScreen, lines: reports.map(humanReport) })) === 0
            ? 'resume'
            : 'rollback',
    };
