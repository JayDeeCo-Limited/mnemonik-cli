...
const missingNode = 'The Node installation that Mnemonik was set up with has moved; run the install command again.';
const same = (a, b) => a?.value === b?.value && a?.kind === b?.kind;
...
function contents(entry, node, platform) {
    if (!isAbsolute(node) ||
        /[\r\n\0]/.test(entry + node) ||
...
function pinnedOwned(match, entry, decode) {
...
function owned(bytes, entry, platform) {
    ...
    const quoted = '((?:[^"\\\\$`]|\\\\[\\\\"$`])*)';
    const pinned = new RegExp(`^#!/bin/sh\\n# --mnemonik-owner=cli\\nif \\[ ! -x "${quoted}" \\]; then\\n  echo '${missingNode.replace('.', '\\.')}' >&2\\n  exit 1\\nfi\\nexec "${quoted}" "${quoted}" "\\$@"\\n?$`).exec(text);
    if (pinned)
        return pinnedOwned(pinned, entry, decode);
    const legacy = new RegExp(`^#!/bin/sh\\n# --mnemonik-owner=cli\\nexec node "${quoted}" "\\$@"\\n?$`).exec(text);
    const legacyTarget = legacy?.[1];
    return legacyTarget !== undefined && decode(legacyTarget) === entry;
}
export function launcherPathAction(options = {}) {
    const { platform, env } = locations(options);
    if (platform === 'win32')
        return 'Open a new terminal, then run mnemonik status.';
...
export async function launcherStatus(options = {}) {
...
export class LauncherError extends Error {
    ...
    constructor(launcher, reason = 'launcher_not_ours') {
...
export async function ensureLauncher(options = {}) {
    const loc = locations(options);
    ...
    return withLock(loc.record, 1000, async (assertOwned) => {
        ...
        if (status.ownership === 'not_ours')
            throw new LauncherError(status);
        ...
        if (record.path !== loc.path || record.entry !== loc.entry || record.phase === 'removing')
            throw new LauncherError({ ...status, action: `${fallback} uninstall` }, 'launcher_record_conflict');
        const current = await bytesAt(loc.path);
        const needsWrite = current === null || !current.equals(desired);
        if (status.ownership === 'ours' &&
        ...
        const save = () => atomicWrite(loc.record, Buffer.from(JSON.stringify(record)), undefined, assertOwned);
        await save();
        if (needsWrite) {
            await mkdir(loc.directory, { recursive: true });
            await assertOwned();
            if (current !== null) {
                if ((await launcherStatus(options)).ownership !== 'ours')
                    throw new LauncherError(status);
                await rm(loc.path);
            }
        ...
        if (record.windowsPath) {
            ...
            if (!same(current, record.windowsPath.after)) {
                if (!same(current, record.windowsPath.before))
                    throw new LauncherError({ ...status, action: `${fallback} uninstall` }, 'user_path_changed');
                await assertOwned();
...
export async function removeLauncher(options = {}) {
    const loc = locations(options);
    return withLock(loc.record, 1000, async (assertOwned) => {
        const status = await launcherStatus(options);
        const bytes = await bytesAt(loc.record);
        if (!bytes && status.ownership === 'missing')
            return false;
        if (status.ownership === 'not_ours')
            throw new LauncherError(status);
        const record = bytes
        ...
        if (change && !same(change.before, change.after)) {
            ...
            const restored = same(current, change.after)
                ...
                : current && {
                    ...current,
                    value: removeWindowsPathEntry(current.value, loc.directory, preferredIndex ?? -1, loc.env),
                };
            if (!same(current, restored))
                await writeWindowsUserPath(current, restored, options);
        }
        await assertOwned();
        // Recheck ownership before removing the file or scheduling its removal.
        if ((await launcherStatus(options)).ownership === 'not_ours')
            throw new LauncherError(status);
        if (loc.platform === 'win32' && loc.env.MNEMONIK_LAUNCHER === '1') {
            ...
            const helper = Buffer.from(`Start-Sleep -Seconds 2; Remove-Item -LiteralPath '${loc.path.replaceAll("'", "''")}' -Force`, 'utf16le').toString('base64');
            await new Promise((resolve, reject) => {
                const child = (options.spawn ?? nodeSpawn)('powershell.exe', [
                    '-NoProfile',
                    '-NonInteractive',
                    '-Command',
                ...
                child.once('exit', (code) => code === 0 ? resolve() : reject(new Error(`launcher_removal_helper_${code ?? 'signal'}`)));
            });
