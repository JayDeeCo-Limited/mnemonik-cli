...
const macComputerName = async () => (await promisify(execFile)('/usr/sbin/scutil', ['--get', 'ComputerName'], {
    encoding: 'utf8',
    timeout: 2000,
})).stdout;
/**
 ...
 */
export async function machineName(platform = process.platform, computerName = macComputerName) {
    if (platform === 'darwin') {
        const name = await computerName().then((value) => value.trim(), () => '');
        if (name)
            return name;
    }
    return hostname();
