...
export const REPOSITORY_APPROVAL_INSTRUCTION = 'Open the link below to choose your project folders.';
...
export async function runDeviceFlow(options) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        if (attempt === 0) {
            options.print(refused
                ? 'That approval request was refused. Starting a new one.'
                : 'That approval request expired. Starting a new one.');
            continue;
        }
        options.print(refused ? 'The new approval request was refused.' : 'The new approval request expired.');
