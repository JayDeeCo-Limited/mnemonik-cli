...
export function createCredentialAdapter(options = {}) {
    ...
    const reference = (name) => `mnemonik.credentials.v1:${name}`;
    ...
    async function requestRotation(familyId, request) {
        const started = now();
    ...
    function rotateLocked(familyId, transport) {
        const familyPaths = credentialPaths(files.stateDir, familyId);
        return rotateStored(familyPaths.record, familyId, () => readFamily(familyId), (current) => transport.rotateFamily(familyId, current.refreshToken), (body) => ('access_token' in body ? body : null), (current, body) => persistFamily(current.componentKind, body, {
            schemaVersion: 1,
            kind: 'component',
    ...
    async function forget() {
        ...
        for (const name of await files.list(paths.familyRecords)) {
            ...
            const record = await readRecord(recordPath, 'component');
            if (!record)
                continue;
        ...
        const root = await readRecord(paths.rootRecord, 'root-binding');
        if (root) {
...
export async function fetchWithHookCredential(credential, url, init) {
    ...
    if (!credential || credential.unavailable)
        return unavailable();
    try {
        const result = await credential.credentials.withCredential(credential.familyId, {
        ...
        credential.unavailable ||= error?.code === 'invalid_grant';
    }
